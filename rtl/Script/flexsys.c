    ...
    for (n = 0; n < N; n += SYS_COLS)
    {
        ...
        for (m = 0; m < M; m += ACCUM_SIZE)
        {
            ...
            for (k = 0; k < K; k += SYS_ROWS)
            {
                ...
                Tile_A_Address = (uint32_t)(A + k + m * KK);
                Tile_B_Address = (uint32_t)(B + n + k * NN + (ksize - 1) * NN);
                Tile_C_Address = (uint32_t)(C + n + m * KK);
                Configure_GEMM(Tile_A_Address, Tile_B_Address, Tile_C_Address, KK, NN, msize, ksize, nsize, first, last);
                while (GEMM_A == 1)
                {
